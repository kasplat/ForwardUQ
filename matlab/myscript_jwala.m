...
row_num = 0;
for i=10:step_size1:40  % changing the middle one is the step size
    for j = i+1:step_size2:60  % step size must be changed in here as well.
         row_num = row_num + 1;
             ...
             for y = 1:101
                if label(x,y,i)>0
                    if va(x,y,i) < thresholds(t_row,1)
                        labels_thresholds(x,y,t_row) = 1;  % low % healthy
                    elseif va(x,y,i) < thresholds(t_row,2)
                        labels_thresholds(x,y,t_row) = 2;  % medium % gray
                    else
                        labels_thresholds(x,y,t_row) = 3;  % high % scar
                    end
                else
                    labels_thresholds(x,y,t_row)=0;
                end
             end
         end
         imshow(labels_thresholds(:,:,t_row),[0,3]);
